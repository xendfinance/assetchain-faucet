...
export class FaucetPage extends React.PureComponent<IFaucetPageProps, IFaucetPageState> {
	...
	constructor(props: IFaucetPageProps, state: IFaucetPageState) {
		...
		this.pageContext = {
			faucetApi: faucetApi,
			showStatusAlert: (level: string, prio: number, body: React.ReactElement) =>
				this.showStatusAlert(level, prio, body),
			...
			showNotification: (type: string, message: string, time?: number | boolean, timeout?: number) =>
				this.showNotification(type, message, time, timeout),
						...
						<div className="faucet-body">
							<Router>
								<Routes>
									...
									<Route path="/mine/:session" element={<MiningPage />} />
									<Route path="/claim/:session" element={<ClaimPage />} />
									<Route path="/details/:session" element={<DetailsPage />} />
									<Route path="/status" element={<FaucetStatusPage />} />
	...
	private renderDialogs(): React.ReactElement[] {
		return this.state.dialogs.map((dialog) => <FaucetDialog key={dialog.id} {...dialog.dialog} />);
	}
	...
	private hideDialog(dialogId: number): void {
		...
		if (dialog && dialogIdx !== -1) {
			if (dialog.closeFn) dialog.closeFn();
