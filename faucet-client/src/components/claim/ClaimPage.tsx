...
export class ClaimPage extends React.PureComponent<IClaimPageProps, IClaimPageState> {
	...
	constructor(props: IClaimPageProps, state: IClaimPageState) {
		...
		if (claimWsEndpoint.match(/^\//)) claimWsEndpoint = location.origin.replace(/^http/, "ws") + claimWsEndpoint;
	...
	private async refreshSessionStatus() {
		...
		try {
			let sessionStatus = await this.props.pageContext.faucetApi.getSessionStatus(
		...
		if (this.state.sessionStatus) {
			...
			if (this.state.sessionStatus.status === "claiming") {
				...
				if (
					...
					this.state.sessionStatus.claimIdx <= (this.state.claimNotification?.confirmedIdx || 0)
				) {
	...
	private renderClaim(): React.ReactElement {
		...
		return (
			<div>
				<div className="row">
					<div className="col-3">Wallet:</div>
					<div className="col">{this.state.sessionStatus.target}</div>
				</div>
				<div className="row">
				...
				<div className="row">
					<div className="col-3">Timeout:</div>
					<div className="col">
						{this.state.sessionStatus.status === "claimable" ? (
							<span className="claim-timeout">
								{renderDate(
									new Date((this.state.sessionStatus.start + this.props.faucetConfig.sessionTimeout) * 1000),
									true
								)}{" "}
								({renderTimespan(claimTimeout)})
							</span>
						) : (
							<span className="claim-timeout">-</span>
						)}
					</div>
				</div>
	...
	private renderClaimForm(): React.ReactElement {
		return (
			<ClaimInput faucetConfig={this.props.faucetConfig} submitInputs={(claimData) => this.submitClaim(claimData)} />
		);
	}
			...
			<div className="claim-status">
				<div className="alert alert-primary spinner-alert">
					...
					<span className="spinner-text">
						<br />
						You can close this page now. Your claim is queued and will be processed as soon as possible.
					</span>
					<div className="queue-info container">
						<div className="row">
							<div className="col-3">TX-Status:</div>
							<div className="col">
								{this.state.sessionStatus.claimHash ||
								this.state.sessionStatus.claimIdx <= this.state.claimNotification?.processedIdx
									? "Sending"
									: "Queued"}
							</div>
						</div>
								...
								<div className="col">
									<span className="txhash">
										{this.props.faucetConfig.ethTxExplorerLink ? (
											<a
												href={this.props.faucetConfig.ethTxExplorerLink.replace(
													"{txid}",
													this.state.sessionStatus.claimHash
												)}
												target="_blank"
						...
						<div className="row">
							<div className="col-3">Queue Position:</div>
							<div className="col">
								<OverlayTrigger
									placement="auto"
									overlay={
										<Tooltip>
											{(this.state.sessionStatus.claimIdx || 0) - (this.state.claimNotification?.processedIdx || 0) - 1}{" "}
											claims will be processed before yours.
										</Tooltip>
									}
								>
									<span>
	...
	private renderSessionFailed(): React.ReactElement {
		return (
			<div className="claim-status">
				<div className="alert alert-danger">
					Claim failed: {this.state.sessionStatus.failedReason || this.state.sessionStatus.claimMessage}{" "}
					{this.state.sessionStatus.failedCode ? " [" + this.state.sessionStatus.failedCode + "]" : ""}
				</div>
			</div>
		);
	}
			...
			<div className="claim-status">
				<div className="alert alert-success">
					...
					<span className="txhash">
						{this.props.faucetConfig.ethTxExplorerLink ? (
							<a
								href={this.props.faucetConfig.ethTxExplorerLink.replace("{txid}", this.state.sessionStatus.claimHash)}
								target="_blank"
								rel="noopener noreferrer"
							>
								{this.state.sessionStatus.claimHash}
			...
			<div className="result-sharing">
				{this.props.faucetConfig.resultSharing.preHtml ? (
					<div
						...
						dangerouslySetInnerHTML={{
							__html: this.replaceShareMessagePlaceholders(this.props.faucetConfig.resultSharing.preHtml),
						}}
					/>
				) : null}
				{shareEls.length > 0 ? (
				...
				{this.props.faucetConfig.resultSharing.postHtml ? (
					<div
						className="sh-html"
						dangerouslySetInnerHTML={{
							__html: this.replaceShareMessagePlaceholders(this.props.faucetConfig.resultSharing.postHtml),
						}}
					/>
				) : null}
			</div>
	...
	private replaceShareMessagePlaceholders(message: string): string {
		message = message.replace(/{sessionid}/gi, this.state.sessionStatus.session);
		message = message.replace(/{target}/gi, this.state.sessionStatus.target);
		message = message.replace(/{token}/gi, this.props.faucetConfig.faucetCoinSymbol);

		message = message.replace(
			/{amount}/gi,
			toReadableAmount(BigInt(this.state.sessionStatus.balance), this.props.faucetConfig.faucetCoinDecimals)
		);
		let safeUrl = location.protocol + "//" + location.hostname + location.pathname;
		message = message.replace(/{url}/gi, safeUrl);

		let claimableTime =
			this.state.sessionDetails.data["close.time"] || this.props.pageContext.faucetApi.getFaucetTime().getSyncedTime();
		let duration = claimableTime - this.state.sessionStatus.start;
		message = message.replace(/{duration}/gi, renderTimespan(duration));

		message = message.replace(/{hashrate}/gi, () => {
	...
	private async submitClaim(claimData: any): Promise<void> {
		try {
			...
			if (sessionStatus.status === "failed") throw sessionStatus;
			...
			let errMsg: string;
			if (ex && ex.failedCode) errMsg = "[" + ex.failedCode + "] " + ex.failedReason;
			else errMsg = ex.toString();
			this.props.pageContext.showDialog({
				title: "Claim failed",
				body: <div className="alert alert-danger">Could not claim rewards: {errMsg}</div>,
				closeButton: {
					caption: "Close",
				},
			});
