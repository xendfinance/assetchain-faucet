...
export class DetailsPage extends React.PureComponent<IDetailsPageProps, IDetailsPageState> {
	...
	public componentDidMount() {
		if (!this.state.sessionStatus) this.refreshSessionStatus();
	}
	...
	private async refreshSessionStatus() {
		...
		try {
			...
			if (statusDetails) delete sessionStatus.details;
			this.setState(
				{
					loadingStatus: false,
	...
	private renderSession(): React.ReactElement {
		...
		if (this.state.sessionStatus.status === "claimable") {
			...
			this.state.sessionStatus.status === "running" &&
			this.state.sessionStatus.tasks.filter((task) => task.module === "pow").length > 0
		) {
			restoreButton = (
				<button
					className="btn btn-primary action-btn"
		...
		return (
			<div>
				...
				<div className="row">
					...
					<div className="col">{renderDate(new Date(this.state.sessionStatus.start * 1000), true)}</div>
				</div>
				<div className="row">
				...
				<div className="row">
					...
					<div className="col">{this.renderSessionStatus()}</div>
				</div>
				...
				<div className="row details-advanced">
					<div className="col">
						...
						>
							{this.state.showAllDetails ? "hide" : "show"} session details
						</a>
				...
				<div className="row">
					<div className="col">{restoreButton}</div>
					<div className="col-4">
						<button
		...
		return (
			<div className="status-details">
				{this.state.sessionStatus.failedCode ? (
					<div className="row">
						<div className="col-3">Error Code:</div>
						<div className="col">{this.state.sessionStatus.failedCode}</div>
					</div>
				) : null}
				<div className="row">
					<div className="col-3">Error Reason:</div>
					<div className="col">{this.state.sessionStatus.failedReason}</div>
				</div>
			</div>
		);
	}
		...
		return (
			<div className="status-details">
				<div className="row">
					...
					<div className="col">{this.renderTxStatusBadge()}</div>
				</div>
