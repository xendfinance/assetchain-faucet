...
export class MiningPage extends React.PureComponent<IMiningPageProps, IMiningPageState> {
	...
	private initPoWControls() {
		...
		if (
			...
			this.props.pageContext.activeSession.getSessionId() === this.props.sessionId
		) {
			this.faucetSession = this.props.pageContext.activeSession;
			this.props.pageContext.activeSession = null;
		} else this.faucetSession = new FaucetSession(this.props.pageContext, this.props.sessionId);

		let powWsEndpoint = this.props.faucetConfig.modules.pow.powWsUrl || "/ws/pow";
		if (powWsEndpoint.match(/^\//)) powWsEndpoint = location.origin.replace(/^http/, "ws") + powWsEndpoint;
			...
			this.faucetSession.loadSessionInfo().then(
				(sessionInfo) => {
					if (
						...
						sessionInfo.tasks?.filter((task) => task.module === "pow").length > 0
					) {
		...
		if (this.state.loadingError) {
			return (
				<div className="alert alert-danger">
					Can't mine for this session: {typeof this.state.loadingError == "string" ? this.state.loadingError : ""}
					<br />
	...
	private async processSessionError(error: any) {
		...
		if (showDialog) {
			...
			this.props.pageContext.showDialog({
				...
				body: (
					<div className="alert alert-danger">
						{error.data?.code ? "[" + error.data?.code + "] " : ""} {error.data?.message}
					</div>
				),
				applyButton: {
