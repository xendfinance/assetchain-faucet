...
export class MiningPage extends React.PureComponent<
  IMiningPageProps,
  IMiningPageState
> {
    ...
    this.eventListeners = {
      ...
      sessionBalance: {
        ...
        listener: () => {
          this.setState({
            isClaimable:
              this.powSession.getBalance() >= this.props.faucetConfig.minClaim,
          });
  ...
  private initPoWControls() {
    if (!this.props.faucetConfig.modules.pow) return;

    if (
      this.props.pageContext.activeSession &&
      this.props.pageContext.activeSession.getSessionId() ===
        this.props.sessionId
    ) {
      this.faucetSession = this.props.pageContext.activeSession;
      this.props.pageContext.activeSession = null;
    } else
      this.faucetSession = new FaucetSession(
        this.props.pageContext,
        this.props.sessionId,
      );

    let powWsEndpoint =
      this.props.faucetConfig.modules.pow.powWsUrl || "/ws/pow";
    ...
    this.powSession = new PoWSession({
      ...
      showNotification: (
        ...
        return this.props.pageContext.showNotification(
          type,
          message,
          time,
          timeout,
    ...
    this.powMiner = new PoWMiner({
      ...
      workerSrc: {
        [PoWHashAlgo.SCRYPT]:
          "/js/powfaucet-worker-sc.js?" + FAUCET_CLIENT_BUILDTIME,
        [PoWHashAlgo.CRYPTONIGHT]:
          "/js/powfaucet-worker-cn.js?" + FAUCET_CLIENT_BUILDTIME,
        [PoWHashAlgo.ARGON2]:
          "/js/powfaucet-worker-a2.js?" + FAUCET_CLIENT_BUILDTIME,
      },
    });
  }
  ...
  private updateConnectionState(connected: boolean, initial?: boolean) {
    ...
    if (connected && this.connectionAlertId !== null) {
      ...
      this.connectionAlertId = this.props.pageContext.showStatusAlert(
        "error",
        30,
        <ConnectionAlert
  ...
  public componentDidMount() {
    Object.keys(this.eventListeners).forEach((listenerKey) => {
      ...
      eventListener.emmiter.on(
        eventListener.event,
        eventListener.listener as any,
      );
      eventListener.bound = true;
    });
    if (!this.state.loadedSession) {
      this.faucetSession.loadSessionInfo().then(
        (sessionInfo) => {
          if (
            sessionInfo.status === "running" &&
            sessionInfo.tasks?.filter((task) => task.module === "pow").length >
              0
          ) {
            ...
            this.setState({
              loadedSession: true,
              isClaimable:
                this.powSession.getBalance() >=
                this.props.faucetConfig.minClaim,
            });
            FaucetSession.persistSessionInfo(this.faucetSession);
          } else this.processSessionStatusRedirects(sessionInfo);
        },
        (err) => {
          this.setState({
            loadedSession: false,
  ...
  public render(): React.ReactElement<IMiningPageProps> {
    ...
    this.powMiner.setPoWParams(
      this.props.faucetConfig.modules.pow.powParams,
      this.props.faucetConfig.modules.pow.powDifficulty,
      this.props.faucetConfig.modules.pow.powNonceCount,
    );
      ...
      this.props.pageContext.showDialog({
        ...
        body: (
          <div className="alert alert-warning">
            ...
            .<br />
            Do you want to stop mining and loose the rewards you've already
            collected?
          </div>
        ),
        closeButton: {
