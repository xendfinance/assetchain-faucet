	...
	public componentDidMount() {
		if (!this.powMinerStatsListener) {
			this.powMinerStatsListener = (stats: IPoWMinerStats) => {
				...
				if (this.state.workerCountInput === 0) stateChange.workerCountInput = stats.workerCount;
				if (this.state.workerCount !== stats.workerCount) stateChange.workerCount = stats.workerCount;
		...
		return (
			<div className="grid pow-status">
				...
				<div className="row pow-status-other">
					...
					<div className="col-3">
						<div className="status-value">
							{this.state.workerCount} / {this.state.workerCountInput}
						</div>
					</div>
					<div className="col-3 pow-worker-controls">
						<button
							...
							disabled={this.state.workerCountInput >= 32}
							onClick={() => this.onChangeWorkerCountButtonClick(1)}
						>
							+
						</button>
						<button
							type="button"
							className="btn btn-primary btn-sm"
							disabled={this.state.workerCountInput <= 1}
							onClick={() => this.onChangeWorkerCountButtonClick(-1)}
						>
							-
						</button>
				...
				<div className="row pow-status-other">
					...
					<div className="col-6">
						<div className="status-value">
							{toReadableAmount(
								(BigInt(this.state.balance || 0) * 3600n) / BigInt(miningTime || 1),
								this.props.faucetConfig.faucetCoinDecimals,
								this.props.faucetConfig.faucetCoinSymbol
							)}
							/h
				...
				{this.props.faucetConfig.modules.passport ? (
					<div className="row pow-status-other">
						...
						<div className="col-3 pow-passport-controls">
							<button type="button" className="btn btn-primary btn-sm" onClick={() => this.props.openPassportInfo()}>
								Boost
							</button>
						</div>
					</div>
