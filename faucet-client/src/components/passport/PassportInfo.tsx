...
export class PassportInfo extends React.PureComponent<
  IPassportInfoProps,
  IPassportInfoState
> {
  ...
  public componentDidMount() {
    if (!this.state.passportInfo) this.refreshPassportInfo();
  }

  private async refreshPassportInfo() {
    if (this.loadingPassport) return;
    ...
    try {
      let passportInfo = await this.props.pageContext.faucetApi.getPassportInfo(
    ...
    return (
      <div className="pow-boost-info">
        <div className="boost-descr">
          Increase your passport score by verifying stamps on your{" "}
          <a href="https://passport.gitcoin.co/#/dashboard" target="_blank">
            Gitcoin Passport
          </a>
          .
        </div>
        <div className="boost-passport">
          <div className="passport-summary container">
            <div className="row">
              <div className="col-12 col-lg-4">Passport Address:</div>
              <div className="col-12 col-lg-8 summary-val">
                {this.props.targetAddr}
              </div>
            </div>
            <div className="row">
              <div className="col-12 col-lg-4">Passport Score:</div>
              <div className="col-12 col-lg-8 summary-val">
                <span className="passport-score">
                  {this.state.passportInfo.score.score || 0}
                </span>
                {this.props.faucetConfig.modules["passport"].boostFactor ? (
                  <span className="passport-factor">
                    ...
                    <OverlayTrigger
                      placement="bottom"
                      overlay={<Tooltip>{this.renderFactorInfo()}</Tooltip>}
                    >
            ...
            <div className="row passport-refresh">
              ...
              >
                <button
                  ...
                  disabled={
                    this.state.refreshCooldownSec > 0 ||
                    this.state.refreshProcessing ||
                    this.state.manualRefreshRunning
                  }
                >
                  Refresh Passport Automatically
                  {this.state.refreshCooldownSec > 0
                    ? " (" + this.state.refreshCooldownSec + ")"
                    : ""}
                </button>
              </div>
              {this.props.faucetConfig.modules["passport"]
    ...
    return (
      <div className="passport-factor-info container">
        <div className="row header-row">
          <div className="col-6">Min Score</div>
          <div className="col-6">Reward Factor</div>
        </div>
        {Object.keys(
          this.props.faucetConfig.modules["passport"].boostFactor,
        ).map((score) => {
          let factor =
            this.props.faucetConfig.modules["passport"].boostFactor[score];
          return (
            <div key={"score-" + score} className="row score-row">
              <div className="col-6">{score}</div>
              <div className="col-6">{factor}</div>
            </div>
          );
        })}
      </div>
    ...
    return (
      <div className="passport-details container">
        <div className="row details-header">
          <div className="col">Passport Score Details:</div>
        </div>
        {stamps.map((stamp) => {
          return (
            <div key={"stamp-" + stamp.provider} className="row passport-stamp">
              <div className="col-8">{stamp.provider}</div>
              <div className="col-4">
                {stamp.expiration > now && !stamp.duplicate ? (
                  <span>
                    +{" "}
                    {
                      this.props.faucetConfig.modules["passport"].stampScoring[
                        stamp.provider
                      ]
                    }
                  </span>
                ...
                {stamp.expiration <= now ? (
                  <OverlayTrigger
                    ...
                    overlay={
                      <Tooltip>
                        This stamp has been expired. Please refresh it on
                        passport.gitcoin.co
                      </Tooltip>
                    }
                  >
                    <span key="status" className="badge bg-danger">
                      Expired
                    </span>
                  </OverlayTrigger>
                ) : null}
                {stamp.duplicate ? (
                  <OverlayTrigger
                    ...
                    overlay={
                      <Tooltip>
                        This stamp has already been used in the passport for{" "}
                        {stamp.duplicate}
                      </Tooltip>
                    }
                  >
        ...
        <div className="row">
          <div className="col">
            <textarea
              ...
              onChange={(evt) =>
                this.setState({ passportJson: evt.target.value })
              }
        ...
        <div className="row">
          <div className="col">
            <button
              ...
              disabled={
                this.state.manualRefreshRunning ||
                this.state.passportJson.length < 100
              }
            >
  ...
  private setPassportRefreshCooldown(cooldownTime?: number) {
    ...
    let cooldownSec =
      cooldownTime -
      this.props.pageContext.faucetApi.getFaucetTime().getSyncedTime();
  ...
  private onRefreshPassportClick() {
    ...
    this.props.pageContext.faucetApi
      .refreshPassport(this.props.sessionId)
      .then((res: any) => {
      ...
      .catch((err) => {
        ...
        this.setState({
          ...
          refreshError: true,
          refreshStatus:
            (err.error ? err.error : err.toString()) +
            (err.errors ? "\n" + err.errors.join("\n") : ""),
        });
        if (err.cooldown) {
  ...
  private onUploadPassportJsonClick() {
    ...
    this.props.pageContext.faucetApi
      .refreshPassportJson(this.props.sessionId, this.state.passportJson)
      .then((res: any) => {
        if (res.error) throw res;
